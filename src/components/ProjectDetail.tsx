      ...
      <div className="border-b border-slate-200">
        <nav className="-mb-px flex space-x-8 overflow-x-auto">
          <button
            ...
            className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'premissas' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
              }`}
          >
            Simulação de Riscos
          </button>
          <button
            ...
            className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'graficos' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
              }`}
          >
            Visão Gráfica
          </button>
          <button
            ...
            className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'tabela' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
              }`}
          >
          ...
          <button
            ...
            className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'dados' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
              }`}
          >
          ...
          <button
            ...
            className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'sensibilidade' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
              }`}
          >
      ...
      {activeTab === 'dados' && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="space-y-8">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-700 border-b border-slate-100 pb-2 mb-4">Outros Custos (OPEX)</h3>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                {renderInput('% VGV (Obra < 33%)', 'outrosCustosVgvTerc1', 'percent', '0.06')}
                {renderInput('% VGV (Obra < 66%)', 'outrosCustosVgvTerc2', 'percent', '0.04')}
                {renderInput('% VGV (Obra > 66%)', 'outrosCustosVgvTerc3', 'percent', '0.02')}
                {renderInput('Jurídico Obra (R$/mês)', 'custoJuridicoObra', 'number', '15000')}
                {renderInput('Jurídico Pós (R$/mês)', 'custoJuridicoPosObra', 'number', '5000')}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="md:col-span-4 flex items-center space-x-2 mt-2 bg-slate-50 p-3 rounded-lg border border-slate-200">
                  <input 
                    type="checkbox" 
                    id="permutaCashSweep"
                    className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 w-4 h-4 cursor-pointer"
                    checked={project.permutaCashSweep || false}
                    onChange={(e) => updateField('permutaCashSweep', e.target.checked)}
                  />
                  <label htmlFor="permutaCashSweep" className="text-sm font-medium text-slate-700 cursor-pointer">
                    Fazer "Cash Sweep" da Permuta em todas as fases <span className="text-slate-500 text-xs font-normal ml-1">(Se desmarcado, a Permuta aguardará Dívida Sênior pós-obras)</span>
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6 h-fit sticky top-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
              ...
              <div className="space-y-5">
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-sm font-medium text-slate-700">Carrego (Padrão Baixo - R$/m²)</label>
                    <span className="text-sm text-slate-500">R$ {sim.carregoBaixo}</span>
                  </div>
                  <input type="range" min="10" max="50" step="1" value={sim.carregoBaixo} onChange={(e) => setSim({...sim, carregoBaixo: parseInt(e.target.value)})} className="w-full accent-indigo-600" />
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-sm font-medium text-slate-700">Carrego (Padrão Médio - R$/m²)</label>
                    <span className="text-sm text-slate-500">R$ {sim.carregoMedio}</span>
                  </div>
                  <input type="range" min="10" max="60" step="1" value={sim.carregoMedio} onChange={(e) => setSim({...sim, carregoMedio: parseInt(e.target.value)})} className="w-full accent-indigo-600" />
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-sm font-medium text-slate-700">Carrego (Padrão Alto - R$/m²)</label>
                    <span className="text-sm text-slate-500">R$ {sim.carregoAlto}</span>
                  </div>
                  <input type="range" min="10" max="100" step="1" value={sim.carregoAlto} onChange={(e) => setSim({...sim, carregoAlto: parseInt(e.target.value)})} className="w-full accent-indigo-600" />
                </div>
            ...
            <table className="w-full text-sm text-left whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                <tr
                  ...
                  onClick={() => setOutrosCustosExpanded(!outrosCustosExpanded)}
                >
                  <td className="py-2 px-4 text-slate-600 sticky left-0 bg-white z-10 pl-6 flex items-center justify-between border-t border-slate-50">
                    <span className="font-medium">Outros Custos Opex</span>
                    <span className="text-[10px] text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">{outrosCustosExpanded ? '▲' : '▼'}</span>
                  </td>
                  {data.cashFlow.map(cf => {
                    const totalOutros = cf.outrosCustosVgv + cf.custoCarregoEstoque + cf.custosJuridicos;
                    return <td key={cf.month} className={`py-2 px-4 text-right text-rose-600 font-medium ${cf.isConstruction ? '' : 'bg-indigo-50/30'} border-t border-slate-50`}>{totalOutros > 0 ? formatCurrency(-totalOutros) : '-'}</td>
                  })}
                </tr>

                {outrosCustosExpanded && (
                  <>
                    <tr className="bg-slate-50/30">
                      <td className="py-1 px-4 text-slate-400 sticky left-0 bg-white z-10 pl-10 text-[11px] leading-tight">- Outros Custos em % VGV</td>
                      {data.cashFlow.map(cf => <td key={cf.month} className={`py-1 px-4 text-right text-[11px] text-slate-400 ${cf.isConstruction ? '' : 'bg-indigo-50/30'}`}>{cf.outrosCustosVgv > 0 ? formatCurrency(-cf.outrosCustosVgv) : '-'}</td>)}
                    </tr>
                    <tr className="bg-slate-50/30">
                      <td className="py-1 px-4 text-slate-400 sticky left-0 bg-white z-10 pl-10 text-[11px] leading-tight">- Carrego de Estoque</td>
                      {data.cashFlow.map(cf => <td key={cf.month} className={`py-1 px-4 text-right text-[11px] text-slate-400 ${cf.isConstruction ? '' : 'bg-indigo-50/30'}`}>{cf.custoCarregoEstoque > 0 ? formatCurrency(-cf.custoCarregoEstoque) : '-'}</td>)}
                    </tr>
                    <tr className="bg-slate-50/30">
                      <td className="py-1 px-4 text-slate-400 sticky left-0 bg-white z-10 pl-10 text-[11px] leading-tight">- Jurídico e Consultoria (Flat)</td>
                      {data.cashFlow.map(cf => <td key={cf.month} className={`py-1 px-4 text-right text-[11px] text-slate-400 ${cf.isConstruction ? '' : 'bg-indigo-50/30'}`}>{cf.custosJuridicos > 0 ? formatCurrency(-cf.custosJuridicos) : '-'}</td>)}
                    </tr>
                  </>
                )}

                <tr className="bg-slate-50">
                  <td className="py-2 px-4 font-semibold text-slate-900 sticky left-0 bg-slate-50 z-20 shadow-[1px_0_0_0_#e2e8f0] border-y border-slate-200">Resultado Operacional (A)</td>
                  {data.cashFlow.map(cf => {
                    const resOp = cf.receivables + cf.rendimentoCaixa - cf.retPayment - cf.brokeragePayment - cf.constructionCost - cf.outrosCustosVgv - cf.custoCarregoEstoque - cf.custosJuridicos;
                    return <td key={cf.month} className={`py-2 px-4 text-right font-semibold border-y border-slate-200 ${cf.isConstruction ? '' : 'bg-indigo-50/50'} ${resOp >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>{formatCurrency(resOp)}</td>
                  })}
                </tr>
